...
def _tag_to_doc(tag: Tag) -> dict:
    return {"_id": tag.id, "name": tag.name, "org_id": tag.org_id, "created_at": tag.created_at}


def _tag_from_doc(doc: dict) -> Tag:
    return Tag(id=doc["_id"], name=doc["name"], org_id=doc["org_id"], created_at=doc["created_at"])


class MongoTagRepository(MongoRepository, TagRepository):
    collection_name = "conversations_tags"
