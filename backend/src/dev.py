...
_USERS = [
    ...
    ("frank@ddd.dev", "abcd1234", "editor at project (member via sub-scope grant)"),
...
async def seed():
    ...
    # Scope hierarchy
    campaign_handler = CampaignCommandHandler(
        MongoCampaignRepository(db), MongoOrganizationRepository(db)
    )
    ...
    campaign = await campaign_handler.create(
        CreateCampaignCommand(
            name="Q1 2025 Launch",
    ...
    await authz.assign_role(f"user:{carol.id}", "editor", "project", pid, org_id=oid)
    await authz.assign_role(f"user:{dave.id}", "viewer", "campaign", cid, org_id=oid)
    await authz.assign_role(f"user:{eve.id}", "supervisor", "subproject", sid, org_id=oid)
    await authz.assign_role(f"user:{frank.id}", "editor", "project", pid, org_id=oid)
    await authz.assign_role(f"user:{grace.id}", "viewer", "project", pid, org_id=oid)
    await authz.assign_role(f"user:{henry.id}", "editor", "campaign", cid, org_id=oid)
    ...
    # Spread conversation timestamps over the last 180 days
    ...
    for i in range(100):
        ...
        conv_ts = _BASE_TS - timedelta(days=days_ago, hours=hours_offset)
        await conv_handler.create(
            CreateConversationCommand(
                title=f"{topic} — {agent} #{i + 1:02d}",
                content=f"Notes from {topic.lower()} with {agent}.",
                type="review",
                created_by=creator.id,
                conversation_timestamp=conv_ts,
                organization_id=org.id,
    ...
    # Reload enforcer so in-memory rules match the freshly-seeded DB
    from src.iam.infrastructure.enforcer import init_enforcer

    await init_enforcer(db)
